...
**Status:** Open (workaround used, not fixed)
...
**Workaround used tonight:** for Nate's new `league_staff` row, manually looked up his existing `auth.users.id` (from a prior successful login under the same email) and inserted the `auth_person` row directly, rather than relying on the app to pick it up.
**Fix (not yet built):** either (a) re-run `bootstrap_link` on every login regardless of whether personas is already non-empty (cheap — it's a no-op `insert ... on conflict do nothing` for anything already linked), or (b) add a lighter "check for new roles" endpoint/action a coach can trigger from Settings. Option (a) is simpler and removes the need for this manual workaround going forward — flag for Opus review given it touches the auth bootstrap path.
