...
## D31 — Adding a new role/team to an already-active coach never auto-links; requires a manual `auth_person` insert

**Status:** Fixed
**Source:** Discovered 2026-08-14 while adding a `league_staff` persona for Nate Carey, who already had an active, linked `coach`@Madison persona from signing in earlier the same day during pilot kickoff.
**Root cause:** `backend/app/deps.py`'s `get_caller` only calls `app.onboarding.bootstrap_link` (which auto-links every not-yet-linked coach `person` row matching the JWT's verified email) inside `if not personas:` — i.e., only when the caller resolves to **zero** existing personas. A coach who has ever successfully signed in before already has ≥1 resolved persona, so this branch never runs again for them — a brand-new `person` row added later (a second team, a promotion to league_staff, etc.) is silently invisible to them forever, even after signing out and back in, until someone manually inserts the matching `auth_person` row. This is different from (and confirms) the multi-persona case working correctly on **first** login: Andrew's 4 personas all linked together in one pass specifically because they all existed before his very first sign-in.
**Workaround used at the time:** for Nate's new `league_staff` row, manually looked up his existing `auth.users.id` (from a prior successful login under the same email) and inserted the `auth_person` row directly, rather than relying on the app to pick it up.
**Fix:** `bootstrap_link` now also runs unconditionally inside `GET /api/me`'s handler (`backend/app/routes.py`), re-resolving personas if it links anything new — deliberately **not** added to `get_caller` itself, which backs every route (roster/observation/attendance calls included) and would have doubled DB round-trips on the app's hottest path forever to fix a rare edge case. `/api/me` is the one endpoint the frontend actually uses to learn what personas exist (`src/sync.js`'s `syncNow()` awaits it first and caches the result via `saveCachedIdentity`) — called on app open, manual sync, team switch, and sign-in, not on every data call — so a relink attempted there is exactly as fresh as the client can ever observe it. New test in `tests/backend/test_onboarding.py` reproduces the scenario end-to-end against a live Postgres+RLS container.
## D32 — Coach-roster CSV import: full-word Role labels ("Head Coach"/"Team Director") silently downgrade to `coach`, then cascade-fail the whole import if the importing TD's own row is in the file

**Status:** Fixed
**Source:** Andrew, 2026-08-14 — Michael Ballard (newly onboarded team_director, Madison) tried importing his team's real coach roster CSV and got "cannot import roster for that team" on every attempt, while the athlete-file import from the same session succeeded cleanly.
**Root cause, confirmed by reproducing exactly (not guessed):**
1. `src/roster-import.js`'s `parseRole()` only matched the bare substrings `"hc"`/`"td"` — shorthand codes. The real CSV (a live PitZone-style coach export) spells roles out in full: `"Head Coach"`, `"Team Director"`. Neither string contains `"hc"` or `"td"` as a contiguous substring (the space breaks it), so every coach in the file — including Michael's own row — silently mapped to plain `coach` instead of the correct elevated role.
2. Michael's own row appeared partway through the file. Because it matches his existing `person` row by (email, name), it triggers an `UPDATE ... role='coach'` — silently demoting himself from `team_director` mid-transaction.
3. `app_caller_hc_team_ids()` (the RLS helper every `person_insert`/`person_update` policy checks) is `STABLE`, which re-evaluates per-statement, not once per transaction. From that row onward, it re-resolves to *empty* for his session — he's no longer HC/TD as far as RLS is concerned, for the remainder of that same import.
4. Every subsequent row in the file then raises `psycopg.errors.InsufficientPrivilege`, which `backend/app/routes.py`'s `import_roster` correctly (if a bit confusingly, from the coach's side) surfaces as `403 "cannot import roster for that team"`. `rls_connection`'s transaction rolls back on any exception, so nothing partial ever lands — confirmed nothing landed on Madison from any of Michael's failed attempts.
**Confirmed via full simulation** against Michael's real file and real identity (rolled back, nothing written): rows 1–10 succeed, row 11 (his own) "succeeds" but silently demotes him, row 12 fails with the exact error text he saw. Not a guess — reproduced end to end before writing the fix.
**Fix:** `parseRole()` now checks the full-word labels (`"head coach"`, `"team director"`) first, falling back to the bare `"hc"`/`"td"` codes for any file that already uses those, then `coach` for everything else (`"Ride Leader"`, `"Group Ride Coordinator"`, blank, etc.). Frontend-only change — no backend/RLS change needed. New unit tests in `tests/unit/roster-import.test.js` lock in both the full-word cases and that `"Ride Leader"`/`"Group Ride Coordinator"` (real values from this same file) still correctly fall through to `coach`.
**Not fixed (separate, smaller, not urgent):** the 403 message itself gives no hint that the real cause was a mid-batch self-demotion — a coach seeing "cannot import roster for that team" today has no way to know it means "check whether your own row is in this file." Worth a clearer error message eventually, but the root-cause fix above prevents the scenario from happening at all, so not chasing the message wording tonight.
