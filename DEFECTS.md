...
**Status:** Open
**Source:** Andrew, 2026-08-14 — reported as the team switcher "button click doesn't work and can't select the team" for `andrew@idahomtb.org` on prod, immediately after the D26 team switcher's `selectPersona()` triggered a re-sync.
**Root cause (confirmed, not the switcher itself):** at the time, prod's Postgres was missing the `practice`/`attendance` tables (migration `0010_practice_attendance.sql` had never been applied there — see the deploy-gap incident this session). `GET /api/practices` and `GET /api/attendance` both 500'd. `src/sync.js`'s `syncNow()` fetches roster/observations/confirmed-levels/practices/attendance via a single `Promise.all([...])` (`src/sync.js:156-162`) — `Promise.all` is all-or-nothing, so the two broken calls failing threw the WHOLE pull away, including the roster/observations/confirmed-levels that had already succeeded. `selectPersona()` (`src/main.js:237-269`) closes the modal and flashes "Switched to \<team\>" *before* awaiting this `runSync()`, so the switch itself visibly "worked" while the team's roster silently failed to load — reading exactly like a broken button from the coach's side.
**Confirmed NOT the cause:** the click wiring (`data-a="open-team-switcher"` / `data-m="save-team-switch"`) and `personaRoleLabel()`/`resolveActivePersona()` (both correctly handle `league_staff`, the persona role newly in play here) were all checked and are correct.
**Status of the acute trigger:** migration `0010` has since been applied to prod; this specific failure mode is no longer reachable today. Not re-tested against a live broken endpoint after the fact.
**Fix (not yet built):** swap the pull's `Promise.all` for `Promise.allSettled`, save whatever DID succeed, and only report the ones that failed in `result.error` — one broken endpoint (a future migration gap, a transient 500, etc.) should degrade gracefully, not silently discard everything else that came back fine.
