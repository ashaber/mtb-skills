...
**Confirmed the "no email at all" version needs zero schema/RLS changes** — checked both backend and frontend for any hidden assumption that a `coach`-role row always has an email or `ride_group_id` set; there is none, in either. But the actual preferred short-term fix (below) — load the email now for future re-import matching, but keep login blocked regardless — does need one small real addition.

- **L1, no email yet:** `person.role = 'coach'` (displays correctly as a coach everywhere, never conflated with a student athlete), no `email`, no `ride_group_id`, `tags: ['L1']`. Not signing in isn't a permission the app grants or withholds — it's structural: `bootstrap_link`'s email-match has nothing to match against.
- **L1, preferred short-term fix — email loaded now, login still blocked:** the current bootstrap has no concept of "email present but not yet authorized" — needs a new boolean, e.g. `person.login_enabled` (default true for anyone an HC/TD adds normally; explicitly false for L1 rows), checked alongside the email match in `app.onboarding.bootstrap_link`. Small and additive, doesn't touch any existing policy or endpoint — but it is real schema work, not a zero-code fact like the no-email case.
- **Promotion to L2:** HC/TD confirms/sets the email, flips `login_enabled` true, assigns a `ride_group_id`, updates `tags: ['L2']` — the same `person_update` (HC/TD-only, already-existing) action used for any roster edit. No new endpoint, no new policy beyond the one boolean above.
