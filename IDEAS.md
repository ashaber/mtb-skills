...
- **Email deliverability — the part most likely to bite:** Supabase's default built-in email sending has low rate limits and reads as generic/spammy. **Correction (2026-08-14): Resend is NOT actually integrated anywhere in this codebase** — `IDEA-024` only *decided* on Resend for a separate, unbuilt engagement-report-delivery feature; zero supporting code exists (confirmed via repo-wide grep). Worth configuring Supabase's custom SMTP (Resend or otherwise) to replace Supabase's default sender, for reliability and a recognizable "from" address — but that's a Supabase-dashboard config step, not something this app's code does or needs to know about.

**Built 2026-08-14 (both halves of the feature):**
- **Self-serve UI, exactly as scoped above:** `signInWithMagicLink(email)` (`src/auth.js`), the email input + "Email me a sign-in link" button in Settings → Account (`src/views.js`, coexists with the Google button), and the `sign-in-magic-link` action handler (`src/main.js`). Covered by new Vitest coverage in `tests/unit/auth.test.js` and `tests/unit/views.test.js` — no new Playwright e2e (this project has no browser-level test infrastructure for ANY auth-gated feature yet, including the pre-existing Google button; see `tests/e2e/test_team_switcher.py`'s docstring).
- **Admin-triggered "invite" half** (the answer to "can I just send the invite after creating the account"): `scripts/send_invite.py`, a standalone script (same category as `scripts/engagement_report.py` — no new web-reachable endpoint/admin surface). Looks up the target email against `person`/`COACH_ROLES` first (catches a typo before burning a rate-limited send), then calls the exact same public `POST /auth/v1/otp` endpoint the self-serve button uses — anon key only, no service-role key involved.
- **Not yet done:** Supabase dashboard config (enabling the Email/OTP provider + redirect URL allowlist on ITG and prod) and custom SMTP setup — both manual, dashboard-side steps outside this codebase. Andrew's own `live.com` test-account validation (his blocking note below) is still pending too.

**Blocking on:** Andrew setting up a `live.com` test account to validate end-to-end before this ships (his own note).
...
**Addendum (2026-08-17), Andrew's follow-up refinements:**

1. **RLS mechanism, clarified — not a master/replica table.** Postgres RLS supports multiple SELECT policies per table for the same role; they're OR'd together, not replaced — so this is "add one more narrowly-scoped policy alongside the existing tight one," not a data-duplication architecture change (which would introduce its own sync-drift risk for no benefit). This idea actually needs it in **two** places:
   - **`ride_group`** — today a plain `coach` can only see their *own* group (`SECURITY.md`'s RBAC table: "Ride groups | Read: own group only"), so an L2/L3 floating coach can't even browse group names to pick one yet. New policy: any coach can SELECT `(id, name)` for every `ride_group` on their *own team* (not just their assigned one) — safe to widen, `ride_group` carries no sensitive data.
   - **`person`** — for the lead-picks-the-floater flow below, a group lead needs to see *unassigned* coach rows on their team. New policy, deliberately narrow: `role in ('coach', 'coach_l1') and ride_group_id is null and team_id = <caller's own team>` — not general roster visibility, just enough to identify who's floating.
2. **Access level resolved — population-dependent, not one rule:**
   - **L2/L3 floating coaches** (already have real login/access): the original self-select-then-lead-confirms flow, full read+write on whatever group accepts them for that practice.
   - **L1 floaters** (no app access at all, per `[[IDEA-033]]`'s `coach_l1` design — excluded from `COACH_ROLES`, can't log in): the self-select half of the flow doesn't apply — they have no app to select anything in. Flips to **lead-initiated**: the lead picks an L1 floater from the unassigned-coaches list (new `person` policy above) and marks them present, purely as a record/visibility mechanism, not write access.
   - **New, related but distinct need surfaced:** L1 floaters themselves would benefit from *being rated* — an L3 (or L2) coach assessing the L1 floater's own riding, the coach-progression counterpart to `[[IDEA-033]]`'s L1→L2 path, echoing `[[IDEA-006]]`'s original "Level 3 coach should be evaluating Level 1 and 2 coaches" note. Same "shows up on a practice roster" mechanism, opposite data direction (L1 is rated, not rating) — not yet scoped, tracked here as related.
3. **Lead discretion at accept-time:** rather than (or alongside) the tier-based default above, show a read/write toggle right on the attendance-accept action itself, so the lead decides per-coach, per-practice — e.g. an L2/L3 floater the lead doesn't yet fully trust could still be accepted read-only for one practice. Turns the population split into a *default*, not a hard rule; the lead's in-the-moment judgment can override it either direction.
