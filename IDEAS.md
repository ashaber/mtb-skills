...
- **Validated end-to-end on ITG, 2026-08-17:** Andrew seeded a real `outlook.com` test coach (`andrew.shaber@outlook.com`, Boise/Droid ride group) and successfully signed in via the self-serve button — confirmed working through Supabase's default mailer (custom SMTP deliberately deferred, see `docs/PHASE3_INFRA_SETUP.md` section 4e-ii: `idahomtb.org` isn't Andrew's to verify a sender on, and the LLC's `aristosevents.com` is both mid-cleanup and a poor brand fit regardless). Default mailer's confirmed rate limit — 2 emails/hour, 30 SMS/hour per project — is fine for inviting the two blocked pilot coaches, not for real scale.
- **Not yet done:** promoting this deploy to prod (ITG is live as of `d4bf0bc`; prod is still 8 commits behind as of this writing), and actually inviting the two real blocked pilot coaches (`dcacioppo@live.com`, `j_fiedler@live.com`) once prod is current — either self-serve or via `scripts/send_invite.py`. Custom SMTP/a real verified sending domain remains an open, non-blocking fast-follow (see the infra runbook).
